...
void SpaceShooterGame::render()
{
	...
	//Render THE FKING SPACESHIP WOOOOOOOO!!!!!
	m_list_materials.clear();
	...
	//Render ass-te-roids (Insert mokey references)
	m_list_materials.clear();
...
void SpaceShooterGame::updateCamera()
{
	...
	Vector3D new_pos = m_world_cam.getTranslation() + world_cam.getZDirection() * (m_forward * 0.2f);

	//Camera start position
	new_pos = new_pos + world_cam.getXDirection() * (m_rightward * 0.2f);
	...
	m_proj_cam.setPerspectiveFovLH(1.57, ((float)width / (float)height), 0.1f, 600.0f);
}
...
void SpaceShooterGame::updateThirdPersonCamera()
{
	...
	m_proj_cam.setPerspectiveFovLH(1.57, ((float)width / (float)height), 0.1f, 5000.0f);
}
...
{
	// Set the vertices of the shape to the pipeline
	GraphicsEngine::get()->getRenderSystem()->getImmediateDeviceContext()->setVertexBuffer(mesh->getVertexBuffer());

	// Set the indices of the shape to the pipeline
...
void SpaceShooterGame::onSize()
{
	...
	if (GraphicsEngine::get()->getRenderSystem()->getImmediateDeviceContext())
	{
...
//Decreasing m_delta_time will make the view move slower
//+= means inverted, -= means view will move with the mouse, which we do not want... weird but just go with it future me
